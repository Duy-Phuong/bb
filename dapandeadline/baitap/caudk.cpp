...
// Cách 1: Không tổng quát hóa chương trình
double tinhTienTaxi_Cach1(int km)
{
	double sotien = 0;
	...
	else // km > 1
	{
		if (km <= 5)
		{
			sotien = 1 * 15000 + (km - 1) * 13500;
		}
		else // km > 5
		{
			sotien = 1 * 15000 + 4 * 13500 + (km - 5) * 11000;
...
// Cách 2: Tổng quát hóa chương trình bằng cách đặt các hằng ký hiệu
#define NGUONG_1 1
#define NGUONG_2 5
#define NGUONG_3 120
#define TI_LE_GIAM 0.1
#define GIA_1 15000
#define GIA_2 13500
#define GIA_3 11000
double tinhTienTaxi_Cach2(int km)
{
	double sotien = 0;
...
int giaiPhuongTrinhBac3DoiXung_Cach2b(float A, float B, float& x1, float& x2, float& x3)
{
	...
	case MOT_NGHIEM:
		...
		break;
	case HAI_NGHIEM:
		if (x2 != x1 && x3 != x1)	// nếu cả 2 nghiệm khác x1
		{
			songhiem++;				// thì mới tính
		}
		break;
